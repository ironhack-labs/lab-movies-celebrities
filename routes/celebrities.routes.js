...
router.post('/create', (req, res) => {
    ...
    if (name.length === 0 || occupation.length === 0 || catchPhrase.length === 0) {
        res.render('celebrities/new-celebrity', { name, occupation, catchPhrase, errorMsg: 'All fields must be completed' })
        return
    }
...
router.post('/:id', (req, res) => {
    ...
    if (name.length === 0 || occupation.length === 0 || catchPhrase.length === 0) {
        res.render('celebrities/edit-celebrity', {id, name, occupation, catchPhrase, errorMsg: 'All fields must be completed'})
        return
    }
